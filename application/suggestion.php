...
class Suggestion extends CI_Controller {
  ...
  public function add() {
    if ($this->form_validation->run('suggestion') == FALSE) {
      ...
      $this->template->load('mondou_default', 'suggestion/add', $data);
    } else {
      ...
      if (!empty($_POST)) {
        ...
        if (!empty($id)) {
          redirect('mondou/suggestion/add_next/' . $id);
        }
  ...
  public function add_next($id = 0) {
    ...
    $this->template->load('mondou_default', 'suggestion/add_next', $data);
  }
  ...
  public function edit($id = 0) {
    if ($this->form_validation->run('suggestion') == FALSE) {
      ...
      $this->template->load('mondou_default', 'suggestion/edit', $data);
    } else {
      if (!empty($_POST)) {
